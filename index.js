...
switch(command) {
    ...
    date = d.toLocaleDateString(locale, {
      ...
      year: 'numeric'
					...
					await costum(help(pushname, prefix, botName, ownerName, time, kyun, uptime, weton, week, date, totalchat, sender, _registered), text, tescuk, cr)
					break
				case 'iklan':
				case 'iclan':
					kev.sendMessage(from, kevgrup(pushname, prefix, sender, botName, ownerName), text)
					break
				case 'jumlahhuruf':
				case 'reverse':
    if (!isRegistered) return reply(ind.noregis())
...
break
				...
				if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
					await costum(kodebahasa(pushname, prefix, botName, ownerName, sender, _registered), text, tescuk, cr)
					break
				case 'jadwaltv':
				case 'jadwalTV':
					mb = body.slice(10)
