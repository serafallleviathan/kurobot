...
//AFK NGULI
	if (isGroup) {
                 if (checkAfkUser(mentionedJid, _afk)) {
                    const getId = getAfkId(mentionedJid, _afk)
...
switch(command) {
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					await costum(help(pushname, prefix, botName, ownerName, time, kyun, uptime, reqXp, totalchat, getLevelingLevel, sender, _registered, uangku), text, tescuk, cr)
					break
				case 'donasi':
				case 'donate':
...
break
			...
			break 
				...
				case 'ytsearch':
					...
					anu = await fetchJson(`http://zekais-api.herokuapp.com/yts?query=${body.slice(10)}`, {method: 'get'})
...
*/
			...
			kev.sendMessage(from, thumb, image, {quoted: mek, caption: sket})
			    ...
			    var kode = tr.split("|")[0];
			    var quer = tr.split("|")[1];
			    anu = await fetchJson(`http://api.lolhuman.xyz/api/translate/auto/${kode}?apikey=2f12dec349a4bea409e61ff7&text=${quer}`, {method: 'get'})
			    tl = `*From :* ${anu.result.from}\n\n*To :* ${anu.result.to}\n\n*Original :* ${anu.result.original}\n\n*Translated :* ${anu.result.translated}\n\n*Pronunciation :* ${anu.result.pronunciation}`
			    reply(tl)
			    break
				case 'quotes':
				if (isBanned) return reply(ind.baned())
		...
		case 'halloweentext':
		    ...
		    break
				...
				case 'hahi':
				kev.on('qr', qr => {
                const jalh = qrcode.generate(qr, { small: true })
                kev.sendMessage(from, jalh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
                }) 
                break
				case 'qr':
				 if (!isPrem) return reply(ind.premium())				
            ...
            break
				...
				if (!isQuotedSticker) return reply('Reply atau Tag sticker yang mau dijadiin gambar kak >_<')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						kev.sendMessage(from, buffer, gif, {quoted: mek, caption: 'nih kak [(^.^)]'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
...
// PREMIUM
            ...
            case 'next':
				...
				if (!isPrem) return reply(ind.premium())
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/sfw/cry?apikey=syahri2k21`, {method: 'get'})
				...
				if (!isPrem) return reply(ind.premium())
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/sfw/kiss?apikey=syahri2k21`, {method: 'get'})
					reply('「❗」KASIH JEDA 1 MENIT HABIS INI YA KAK')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
				...
				if (!isPrem) return reply(ind.premium())
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/sfw/hug?apikey=syahri2k21`, {method: 'get'})
					reply('「❗」KASIH JEDA 1 MENIT HABIS INI YA KAK')
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const jawab = `${anu.result.jawaban}`
					...
					break
					case 'jawab':
					if (!jawab) return reply('jawaban benar!')
                    kev.sendMessage(from, 'selamat anda mendapatkan .... xp', text) 
                    break
                    case 'umur':
...
break					
		   ...
		   await limitAdd(sender)
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
					...
					kev.sendMessage(from, mediaa, argts, fakeimage, { quoted : mek})
				} else {
					reply(from, `Kirim gambar atau reply dengan caption ${prefix}fakethumb caption`, { quoted : mek})
				}
				break
				case 'setthumb':
				boijj = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
				delbb = await kev.downloadMediaMessage(boijj)
				fs.writeFileSync(`./assets/halo.jpg`, delbb)
				kev.sendMessage(from, `Sukses`, fake)
				break
				case 'addstik':
...
*/
		...
		case 'ytmp4':   
	          if (!isRegistered) return reply(ind.noregis())
                ...
                reply(ind.download)
                mp4 = body.slice(7)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp4?q=${mp4}&apikey=apivinz`)
               if (anu.error) return reply(anu.error)
                 infomp4 = `*Lagu Ditemukan!!!*\nJudul : ${anu.result.title}\nSource : ${anu.result.source}\nUkuran : ${anu.result.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM YA KAKA*`
                buffer = await getBuffer(anu.result.thumbnail)
                videolo = await getBuffer(anu.result.url_video)
                kev.sendMessage(from, buffer, image, {quoted: mek, caption: infomp4})
                kev.sendMessage(from, videolo, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})
                await limitAdd(sender)
                break
		case 'ytmp3':
                    if (!isRegistered) return reply( ind.noregis())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					kev.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					krv.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
					await limitAdd(sender)
					break
		case 'tiktok':
		if (args.length < 1) return reply('Urlnya mana um?')
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(ind.download())
			anu = await fetchJson(`http://api.lolhuman.xyz/api/tiktok?apikey=847de7716f17a51eeba4235c&url=${body.slice(8)}`, {method: 'get'})
			if (anu.error) return reply(anu.error)
			sket = `â�� *Title* : ${anu.result.title}\nâ�� *Filesize* : ${anu.result.duration}\n\nTunggu Bentar Ya Kak, Video Lagi Di Kirim...`
			thumb = await getBuffer(anu.result.thumbnail)
			kev.sendMessage(from, thumb, image, {quoted: mek, caption: sket})
			buffer = await getBuffer(anu.result.link)
			kev.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek})
					await limitAdd(sender)
			break
		case 'joox':
			    if (isBanned) return reply(ind.baned())    
...
break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    ...
				    if (args[0] == 'easy') {
					pertnyan = Math.ceil(Math.random() * 10)
					pertnyan1 = Math.ceil(Math.random() * 10)
				...
				var haho = hahj.split(" ")[0];
				var hahk = hahj.split(" ")[1];
...
// display the result
kev.sendMessage(from, `${number1} ${haho} ${number2} = ${result}`, text)
break
					case 'slow':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					kev.sendMessage(from, wban, text, {quoted: mek, detectLinks: false})
                        console.log(res)
                    })
					await limitAdd(sender)
					break
